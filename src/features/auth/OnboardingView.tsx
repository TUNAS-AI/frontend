...
function FarmStep({ farm, onFarmChange, onAddWindow }: { farm: FarmDraft; onFarmChange: <Key extends keyof FarmDraft>(key: Key, value: FarmDraft[Key]) => void; onAddWindow: () => void }) {
  return <>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <FieldGroup label="Rain protection" helper="Is covered space or a tarpaulin normally available?" reserveHelperSpace><Select value={farm.rainProtectionAvailable} onChange={(event) => onFarmChange("rainProtectionAvailable", event.target.value as FarmDraft["rainProtectionAvailable"])}><option value="unknown">Not recorded</option><option value="yes">Available</option><option value="no">Not available</option></Select></FieldGroup>
    </div>
...
function FieldsAndCropsStep({ fields, onFieldsChange, onFieldChange }: { fields: FieldDraft[]; onFieldsChange: (fields: FieldDraft[]) => void; onFieldChange: (fieldId: string, update: Partial<FieldDraft>) => void }) {
  const updateBatch = (field: FieldDraft, batchId: string, update: Partial<FieldDraft["cropBatches"][number]>) => onFieldChange(field.id, { cropBatches: field.cropBatches.map((item) => item.id === batchId ? { ...item, ...update } : item) });
  return <>
    {fields.map((field, fieldIndex) => <section key={field.id} className="grid gap-5 rounded-lg border border-forest-200 bg-field-50/40 p-4 sm:p-5" aria-labelledby={`field-heading-${field.id}`}>
      <div className="flex items-center justify-between gap-3"><h2 id={`field-heading-${field.id}`} className="text-lg font-extrabold">Field {fieldIndex + 1}</h2><Button type="button" variant="dangerOutline" size="sm" icon={<Trash2 aria-hidden="true" />} disabled={fields.length === 1} onClick={() => onFieldsChange(fields.filter((item) => item.id !== field.id))}>Remove field</Button></div>
      ...
      <section className="grid gap-4 rounded-lg border border-leaf-300 bg-card p-4" aria-labelledby={`crop-batches-heading-${field.id}`}><h3 id={`crop-batches-heading-${field.id}`} className="text-sm font-bold text-forest-700">Shallot crop batches</h3><div className="grid gap-4">{field.cropBatches.map((batch, batchIndex) => <div key={batch.id} className="grid gap-3 rounded-md border bg-background p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold">Batch {batchIndex + 1}</p><Button type="button" size="sm" variant="dangerOutline" icon={<Trash2 aria-hidden="true" />} disabled={field.cropBatches.length === 1} onClick={() => onFieldChange(field.id, { cropBatches: field.cropBatches.filter((item) => item.id !== batch.id) })}>Remove</Button></div><div className="grid gap-3 sm:grid-cols-2"><FieldGroup label="Variety"><Input value={batch.variety} onChange={(event) => updateBatch(field, batch.id, { variety: event.target.value })} placeholder="Bima Brebes" /></FieldGroup><FieldGroup label="Planting date"><Input type="date" value={batch.plantingDate} onChange={(event) => updateBatch(field, batch.id, { plantingDate: event.target.value })} /></FieldGroup><FieldGroup label="Readiness" required><Select value={batch.readinessStatus} onChange={(event) => updateBatch(field, batch.id, { readinessStatus: event.target.value as typeof batch.readinessStatus })}><option value="" disabled>Select readiness</option><option value="READY">Ready</option><option value="NOT_READY">Not ready</option></Select></FieldGroup></div><FieldGroup label="Batch notes"><Textarea className="min-h-20" value={batch.notes} onChange={(event) => updateBatch(field, batch.id, { notes: event.target.value })} placeholder="Farmer-reported context for future missions." /></FieldGroup></div>)}</div><Button type="button" variant="outline" size="sm" className="w-fit" icon={<Plus aria-hidden="true" />} onClick={() => onFieldChange(field.id, { cropBatches: [...field.cropBatches, createCropBatchDraft(field.cropBatches.length)] })}>Add crop batch</Button></section>
    </section>)}
