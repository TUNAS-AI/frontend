...
export function FarmSnapshotPanel({ farmerName, snapshot, snapshotError, snapshotLoading, onRetry }: FarmSnapshotPanelProps) {
  ...
  return <section aria-label="Farm snapshot" className="overflow-hidden rounded-lg border border-forest-300/70 bg-card shadow-farm"><div className="bg-primary px-4 py-3 text-primary-foreground"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-md border border-white/20 bg-white/10"><Sprout className="h-4 w-4" aria-hidden="true" /></span><p className="text-xs font-bold uppercase tracking-[0.14em] text-white">TUNAS AI</p></div><p className="mt-2 flex items-center gap-2 text-sm font-bold text-white/85"><UserRound className="h-4 w-4 text-white/75" aria-hidden="true" />{farmerName}</p></div><div className="grid gap-4 p-4"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-primary">Farm snapshot</p><h2 className="mt-1 text-lg font-extrabold tracking-tight text-foreground">{data.farm.name}</h2><p className="mt-1 flex items-start gap-2 text-sm leading-5 text-muted-foreground"><MapPinned className="mt-0.5 h-4 w-4 shrink-0 text-primary" aria-hidden="true" />{data.farm.location || "Location not recorded"}</p></div><dl className="grid grid-cols-2 gap-3 border-y py-3"><SnapshotMetric icon={<Layers3 aria-hidden="true" />} label="Field blocks" value={data.fieldBlocks.length} /><SnapshotMetric icon={<Sprout aria-hidden="true" />} label="Crop batches" value={data.cropBatches.length} /></dl><Button asChild type="button" size="sm" variant="outline" className="w-full"><Link to="/farm">Manage farm</Link></Button></div></section>;
}
