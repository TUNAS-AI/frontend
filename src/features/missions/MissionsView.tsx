...
export function MissionsView({ missions }: { missions: MissionListItem[] }) {
  return <div className="grid gap-6"><PageHeader eyebrow="Harvest and drying work" title="Missions" description="Review each approved shallot harvest mission and its next planned work." actions={<Button asChild type="button" icon={<Plus aria-hidden="true" />}><Link to="/missions/new">New mission</Link></Button>} />
    <section className="grid gap-4" aria-labelledby="mission-list-heading"><div><h2 id="mission-list-heading" className="text-2xl font-extrabold tracking-tight">Your missions</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Newest missions appear first. Open one to review its plan and schedule.</p></div>{missions.length ? <div className="grid gap-3">{missions.map((mission) => <MissionCard key={mission.missionId} mission={mission} />)}</div> : <EmptyState icon={<ClipboardList className="h-6 w-6" />} title="No missions yet" description="Turn a harvest request into a weather-aware plan, then approve it when you are ready." action={<Button asChild type="button" icon={<Plus aria-hidden="true" />}><Link to="/missions/new">Create a mission</Link></Button>} />}</section></div>;
}
