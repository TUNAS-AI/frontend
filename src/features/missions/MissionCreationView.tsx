...
const labels: Record<string, string> = { fieldBlockId: "Field block", cropBatchIds: "Crop batches", buyerCommitmentId: "Buyer commitment", buyerQuantityKg: "Buyer quantity", marketQuality: "Market quality", plannedHarvestKg: "Planned harvest", plannedDriedKg: "Planned dried weight", deadline: "Deadline", availableWorkerCount: "Available workers", coveredDryingCapacityKg: "Covered drying capacity", notes: "Notes" };
...
export function MissionCreationView() {
  ...
  async function confirm() {
    ...
    try {
      ...
      setError(message);
      if (/expired|previewToken is invalid/i.test(message)) { setPlanPreview(null); setSelectedPlanId(null); }
...
function FactCard({ factKey, candidate, review, onChange }: { factKey: typeof requiredKeys[number]; candidate: MissionPreviewCandidate; review?: MissionFactReview; onChange: (key: EditableFactKey, value: MissionPreviewCandidate["facts"][EditableFactKey]) => void }) {
  ...
  const control = factKey === "fieldBlockId" ? <FieldGroup label={label} required><Select value={typeof value === "string" ? value : ""} onChange={(event) => onChange(factKey, event.target.value || null)} aria-invalid={missing}><option value="">Select a field block</option>{options?.fieldBlocks.map((field) => <option key={field.fieldBlockId} value={field.fieldBlockId}>{field.name}</option>)}</Select></FieldGroup>
    ...
    : factKey === "marketQuality" ? <FieldGroup label={label} required><Select value={value ?? ""} onChange={(event) => onChange(factKey, event.target.value || null)} aria-invalid={missing}><option value="">Select a market-quality grade</option><option value="Grade A">Grade A</option><option value="Grade B">Grade B</option><option value="Grade C">Grade C</option></Select></FieldGroup>
    : factKey === "deadline" ? <DeadlineField value={typeof value === "string" ? value : null} missing={missing} onChange={(next) => onChange(factKey, next)} />
    : <FieldGroup label={label} required><Input type={factKey.includes("Kg") ? "number" : "text"} min={factKey.includes("Kg") ? "0.001" : undefined} step={factKey.includes("Kg") ? "0.001" : undefined} value={value ?? ""} onChange={(event) => onChange(factKey, factKey.includes("Kg") ? (event.target.value === "" ? null : Number(event.target.value)) : (event.target.value || null))} aria-invalid={missing} /></FieldGroup>;
...
function PlanReview({ plans, selectedPlan, onSelect, onRevise, onApprove }: { plans: MissionPreviewPlan[]; selectedPlan: MissionPreviewPlan | null; onSelect: (planId: string) => void; onRevise: () => void; onApprove: () => void }) {
  const orderedPlans = [...plans].sort((left, right) => Number(right.recommended) - Number(left.recommended));
