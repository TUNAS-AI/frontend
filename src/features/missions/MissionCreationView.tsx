...
import { Select } from "@/components/ui/FieldControl";
...
const factHelp: Partial<Record<EditableFactKey, string>> = { fieldBlockId: "Choose where the harvest will take place.", cropBatchIds: "Select every crop batch included in this mission.", readinessConfirmed: "Confirm the selected crop batches are ready before planning.", destination: "Choose what will happen to the harvest after collection.", plannedHarvestKg: "Enter the total fresh crop weight you plan to collect.", deadlineAt: "Set when the harvest mission must be complete." };
...
function FactSummary({ candidate, missing, onChange }: { candidate: MissionPreviewCandidate; missing: readonly string[]; onChange: (key: EditableFactKey, value: MissionPreviewCandidate["facts"][EditableFactKey]) => void }) {
  return <section className="grid gap-4" aria-labelledby="mission-summary-heading"><div><h2 id="mission-summary-heading" className="text-2xl font-extrabold tracking-tight">Mission details</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Review what TUNAS understood. Drying details come from your farm profile.</p></div>{missing.length ? <Alert variant="warning"><CircleAlert aria-hidden="true" /><AlertTitle>Still needed: {missing.map((key) => mvpLabels[key]).join(", ")}</AlertTitle><AlertDescription>Fill the fields below or answer the focused question below.</AlertDescription></Alert> : null}<Card><CardContent className="grid gap-6 pt-5 sm:pt-6"><FactGroup title="Where"><FactCard factKey="fieldBlockId" candidate={candidate} review={reviewFor(candidate, "fieldBlockId")} onChange={onChange} /><FactCard factKey="cropBatchIds" candidate={candidate} review={reviewFor(candidate, "cropBatchIds")} onChange={onChange} /></FactGroup><FactGroup title="Outcome"><FactCard factKey="plannedHarvestKg" candidate={candidate} review={reviewFor(candidate, "plannedHarvestKg")} onChange={onChange} /><FactCard factKey="destination" candidate={candidate} review={reviewFor(candidate, "destination")} onChange={onChange} /><FactCard factKey="deadlineAt" candidate={candidate} review={reviewFor(candidate, "deadlineAt")} onChange={onChange} /></FactGroup><FactGroup title="Confirmation"><FactCard factKey="readinessConfirmed" candidate={candidate} review={reviewFor(candidate, "readinessConfirmed")} onChange={onChange} /><div className="grid min-w-0 content-start gap-3"><div className="flex flex-wrap items-center justify-between gap-2"><label className="font-bold" htmlFor="mission-notes">Additional notes</label><Badge variant="neutral">Optional</Badge></div><p className="min-h-10 text-sm leading-5 text-muted-foreground">Add access, crop condition, or buyer instructions if needed.</p><Textarea id="mission-notes" value={candidate.facts.notes ?? ""} onChange={(event) => onChange("notes", event.target.value || null)} /></div></FactGroup></CardContent></Card></section>;
}

function FactGroup({ title, children }: { title: string; children: ReactNode }) { return <section className="grid gap-3 border-b pb-6 last:border-0 last:pb-0"><h3 className="text-sm font-extrabold uppercase tracking-[0.12em] text-primary">{title}</h3><div className="grid gap-5 sm:grid-cols-2">{children}</div></section>; }
...
function FactCard({ factKey, candidate, review, onChange, optional = false }: { factKey: EditableFactKey; candidate: MissionPreviewCandidate; review?: MissionFactReview; onChange: (key: EditableFactKey, value: MissionPreviewCandidate["facts"][EditableFactKey]) => void; optional?: boolean }) {
  ...
  const control = factKey === "fieldBlockId" ? <Select aria-label={label} required value={typeof value === "string" ? value : ""} onChange={(event) => onChange(factKey, event.target.value || null)} aria-invalid={missing}><option value="">Select a field block</option>{options?.fieldBlocks.map((field) => <option key={field.fieldBlockId} value={field.fieldBlockId}>{field.name}</option>)}</Select>
    : factKey === "cropBatchIds" ? <fieldset aria-label={label}>{candidate.facts.fieldBlockId ? fieldBatches.length ? <div className="grid gap-2 rounded-md border p-3">{fieldBatches.map((batch) => <label key={batch.cropBatchId} className="flex min-h-11 items-center gap-3 text-sm font-medium"><input type="checkbox" className="h-4 w-4 rounded border-input" checked={candidate.facts.cropBatchIds.includes(batch.cropBatchId)} onChange={(event) => onChange(factKey, event.target.checked ? [...candidate.facts.cropBatchIds, batch.cropBatchId] : candidate.facts.cropBatchIds.filter((id) => id !== batch.cropBatchId))} />{batch.label}</label>)}</div> : <p className="flex min-h-11 items-center text-sm text-muted-foreground">No crop batches are available in this field.</p> : <p className="flex min-h-11 items-center text-sm text-muted-foreground">Select a field block first.</p>}</fieldset>
    ...
    : <Input aria-label={label} required={!optional} type="number" min="0" step="0.001" value={typeof value === "number" ? value : ""} onChange={(event) => onChange(factKey, event.target.value === "" ? null : Number(event.target.value))} aria-invalid={!optional && missing} />;
  return <div className="grid min-w-0 content-start gap-3"><div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-bold">{label}{!optional ? <span className="ml-1 text-destructive" aria-hidden="true">*</span> : null}</h4><Badge variant={missing ? "warning" : review?.provenance === "INFERRED" ? "source" : "success"}>{missing ? review?.status === "needs_clarification" ? "Needs clarification" : "Missing" : review?.provenance === "INFERRED" ? "From farm context" : "Confirmed"}</Badge></div><p className="min-h-10 text-sm leading-5 text-muted-foreground">{factKey === "deadlineAt" ? `${factHelp[factKey]} Timezone: ${options?.timezone ?? "mission timezone"}.` : factHelp[factKey]}</p>{control}{review?.provenance === "INFERRED" ? <ConfidenceIndicator level={review.confidence} showScale={false} /> : null}</div>;
}

function EnumField({ label, value, values, onChange }: { label: string; value: unknown; values: string[]; onChange: (value: never) => void }) {
  const optionLabels: Record<string, string> = { IMMEDIATE_SALE: "Immediate sale", CONSUMPTION_STORAGE: "Household or storage", SEED_STOCK: "Seed stock" };
  return <Select aria-label={label} required value={typeof value === "string" ? value : ""} onChange={(event) => onChange((event.target.value || null) as never)}><option value="">Select a destination</option>{values.map((item) => <option key={item} value={item}>{optionLabels[item] ?? item}</option>)}</Select>;
}
...
function IsoDateTimeField({ label, value, timezone, missing, onChange }: { label: string; value: string | null; timezone?: string; missing: boolean; onChange: (value: string | null) => void }) {
  ...
  return <Input aria-label={label} required type="datetime-local" value={localValue} onChange={(event) => update(event.target.value)} aria-invalid={missing} />;
}

function InfeasiblePlan({ blockers, onRevise }: { blockers: string[]; onRevise: () => void }) {
  return <Card className="border-warning/40"><CardHeader><div className="flex items-center gap-2"><CircleAlert aria-hidden="true" /><CardTitle>No feasible plan yet</CardTitle></div><CardDescription>The deterministic planning checks could not produce a safe option. Nothing has been scheduled or changed.</CardDescription></CardHeader><CardContent><ul className="grid gap-2 text-sm leading-6">{blockers.map((blocker) => <li key={blocker}>• {blocker}</li>)}</ul></CardContent><CardFooter className="justify-end"><Button type="button" onClick={onRevise} icon={<ArrowLeft aria-hidden="true" />}>Edit mission details</Button></CardFooter></Card>;
}
