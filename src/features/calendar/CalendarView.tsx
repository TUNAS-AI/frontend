...
export function CalendarView() {
  ...
  const [searchParams, setSearchParams] = useSearchParams(); const handledCallback = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const next = new URLSearchParams(searchParams); next.delete("calendar"); setSearchParams(next, { replace: true });
  ...
  async function sync() { setBusy(true); setConnectionError(null); try { const result = await syncGoogleCalendar(); if (result.failed) setConnectionError(result.failureReason ?? `${result.failed} schedule item${result.failed === 1 ? "" : "s"} could not sync. Try again.`); else toast.success("Google Calendar is up to date"); } catch (reason) { setConnectionError(reason instanceof Error ? reason.message : "We could not sync Google Calendar."); } finally { setBusy(false); } }
