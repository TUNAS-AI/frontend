...
export function CalendarView() {
  ...
  return <div className="grid gap-6"><PageHeader eyebrow="Approved mission work" title="Calendar" description="Pending harvest and drying actions from every approved mission. Google Calendar sync is not active." actions={<div className="flex items-center gap-2"><Button type="button" variant="outline" size="icon" className="border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" aria-label="Previous month" onClick={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() - 1, 1))} icon={<ChevronLeft aria-hidden="true" />} /><Button type="button" variant="outline" size="icon" className="border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" aria-label="Next month" onClick={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() + 1, 1))} icon={<ChevronRight aria-hidden="true" />} /></div>} />
